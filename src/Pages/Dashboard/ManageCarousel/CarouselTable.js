    ...
    <section>
            ...
            <tr key={index} className="border-b border-neutral">
              <td>
                <img
                  src={item?.picture}
                  alt="img"
                  className="h-8 lg:h-16 w-14 lg:w-16 mx-auto my-2 lg:rounded-xl"
                />
              </td>
              <td className="text-center text-sm" title={item?.name}>
                {smallDevice && item?.name?.length > 10
                  ? item?.name.slice(0, 10).concat("...")
                  : !smallDevice && item?.name?.length > 20
                  ? item?.name.slice(0, 20).concat("...")
                  : item?.name}
              </td>
              <td className="text-center" title={item?.tag}>
                {smallDevice && item?.tag?.length > 10
                  ? item?.tag.slice(0, 10).concat("...")
                  : !smallDevice && item?.tag?.length > 20
                  ? item?.tag.slice(0, 20).concat("...")
                  : item?.tag}
              </td>
              ...
              <td className="text-center" title={item?.about}>
                <div className="text-sm opacity-50">
                  {smallDevice && item?.about?.length > 10
                    ? item?.about.slice(0, 10).concat("...")
                    : !smallDevice && item?.about?.length > 20
                    ? item?.about.slice(0, 20).concat("...")
                    : item?.about}
                </div>
              </td>
              <td>
                <div className="flex">
                  <label
                    onClick={() => setEditCarousel(item)}
                    htmlFor="editItemModal"
                    className="text-center mr-5 hover:text-success cursor-pointer"
                  >
                    <svg
                  ...
                  <label
                    htmlFor="DeletePhotoModal"
                    onClick={() => setDeleteCarousel(item)}
                    className="text-center hover:text-red-700 cursor-pointer"
                  >
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      fill="none"
      ...
      {deleteCarousel && (
        <ConfirmDelete
          ...
          refetch={refetch}
          setDeleteItem={setDeleteCarousel}
          fetchTo="carousel"
        />
      )}
      {editCarousel && (
        <EditItemModal
          editItem={editCarousel}
          refetch={refetch}
          setEditItem={setEditCarousel}
          fetchTo="carousel"
        />
      )}
    </section>
